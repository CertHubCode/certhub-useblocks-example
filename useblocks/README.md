...
3. Open the **DeveloperProductivity** monorepo (repo root) in VS Code. Cadence is pinned with
   `ubcode.views.pinnedProject` → `small-pocs/useblocks/sphinx/source/ubproject.toml`
   (see repo-root `.vscode/settings.json`), so Needs Index / Reports stay on Cadence while
   other folders are open. Needs config is shared with Sphinx via
...
4. Ensure generated needs exist, then refresh ubCode:
   ...
   ```bash
   cd small-pocs/useblocks && make sync
   ```
